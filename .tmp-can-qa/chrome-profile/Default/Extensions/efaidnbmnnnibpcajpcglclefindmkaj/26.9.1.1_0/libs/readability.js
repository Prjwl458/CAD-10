...
(function (module) {
  ...
  Readability.prototype = {
    ...
    REGEXPS: {
      ...
      adWords:
        /^(ad(verti[s\u017F]ing|verti[s\u017F]ement)?|pub(licit\xE9)?|werb(ung)?|\u5E7F\u544A|\u0420\u0435\u043A\u043B\u0430\u043C\u0430|Anuncio)$/i,
      loadingWords:
      ...
      while (true) {
        ...
        while (node) {
          ...
          if (
            ...
            this._articleByline = (
              itemPropNameNode !== null && itemPropNameNode !== void 0 ? itemPropNameNode : node
            ).textContent.trim();
            node = this._removeAndGetNext(node);
            continue;
          }
    ...
    /* convert images and figures that have properties like data-src into images that can be loaded without JS */ _fixLazyImages:
      function _fixLazyImages(root) {
        this._forEachNode(
    ...
    _cleanConditionally: function _cleanConditionally(e, tag) {
      ...
      this._removeNodes(this._getAllNodesWithTag(e, [tag]), function (node) {
        ...
        if (
          _toConsumableArray(node.getElementsByTagName("table")).some(function (tbl) {
        ...
        if (this._getCharCount(node, ",") < 10) {
          ...
          var shouldRemoveNode = function shouldRemoveNode() {
            ...
            if (
              ...
              errs.push(
                "Suspiciously short. (headingDensity="
                  .concat(headingDensity, ", img=")
                  .concat(img, ", linkDensity=")
                  .concat(linkDensity, ")"),
              );
            }
            ...
            if ((embedCount === 1 && contentLength < 75) || embedCount > 1) {
              errs.push(
                "Suspicious embed. (embedCount="
                  .concat(embedCount, ", contentLength=")
                  .concat(contentLength, ")"),
              );
            }
            if (img === 0 && textDensity === 0) {
              errs.push(
                "No useful content. (img=".concat(img, ", textDensity=").concat(textDensity, ")"),
              );
            }
            if (errs.length) {
