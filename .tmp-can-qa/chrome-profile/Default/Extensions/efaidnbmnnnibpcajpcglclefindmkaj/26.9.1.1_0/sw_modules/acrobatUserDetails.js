...
O ||
  ((O = new (function () {
    ((this.proxy = r.proxy.bind(this)),
      ...
      (this.updateUserDetails = () => (
        (this.userDetails = {}),
        new Promise((r) => {
          i.getUserInfoFromAcrobat((t) => {
            if (((this.userDetails = t), t && t.errorCode))
              switch (t.errorCode) {
                case "104":
                  e.event(e.e.ADOBE_YOLO_ERROR_PM_OFF);
                  break;
                case "107":
                  e.event(e.e.ADOBE_YOLO_ERROR_LICENSENOTINITIALISED);
                  break;
                case "108":
                  e.event(e.e.ADOBE_YOLO_ERROR_EMPTYACCESSTOKEN);
              }
            (s.setItem("acrobatUserDetailsFetchTimestamp", Date.now()), r());
          });
        })
      )),
      (this.getUserInfoFromAcrobat = async (e, r, t) => {
        const o = await l.checkFeatureEnable({
          flagName: "dc-cv-adobe-yolo",
          storageKey: "adobeYoloEnable",
        });
        if (
          !(s.getItem("isAcrobat") && o && m.ADOBE_YOLO_ENABLED) ||
          s.getWithTTL("adobe-yolo-freeze")
        )
          return t({});
        {
      ...
      (this.launchJumpUrl = (r, l, u) => {
        ...
        i.launchJumpUrl(m, (r) => {
          if (!r || !r.errorCode)
            return (
              s.removeItem("adobe-yolo-failure"),
              e.event(e.e.ADOBE_YOLO_JUMP_SUCCESS, { SOURCE: n }),
              u && u({ result: o.SUCCESS })
            );
          {
            a.removeItem("adobeYoloTabsInfo");
            const i = s.getItem("SFV") || void 0;
            switch (r.errorCode) {
              case "1":
                return (
                  (this.userDetails = {}),
                  e.event(e.e.USER_SIGNED_OUT_ADOBE_YOLO, { SOURCE: n }),
                  u && u({ result: t[1] })
                );
              case "2":
                (s.removeItem("adobe-yolo-failure"),
                  e.event(e.e.STALE_USER_ADOBE_YOLO, { SOURCE: n }));
                const { userName: a, userEmail: l, profileImage: m, userGuid: O } = r;
                return (
                  (this.userDetails = { userName: a, userEmail: l, profileImage: m, userGuid: O }),
              ...
              default:
                return (
                  e.event(e.e.ADOBE_YOLO_JUMP_ERROR_REASON, {
                    SOURCE: n,
                    REASON: t[r.errorCode],
                    VERSION: i,
                  }),
                  s.getItem("adobe-yolo-failure")
                    ...
                    : (s.setItem("adobe-yolo-failure", "true"),
                      u && u({ result: o.ERROR, freeze_reason: t[r.errorCode] }))
                );
            }
          }
        });
  ...
  u.registerModule("acrobatUserDetails", O),
  u.registerHandlers({
    getUserInfoFromAcrobat: O.proxy(O.getUserInfoFromAcrobat),
    launchJumpUrl: O.proxy(O.launchJumpUrl),
  }));
export const userDetailsAcrobat = O;
